...
mlpipeline_minio_access_key = base64.b64encode(bytes(os.environ.get("MINIO_ACCESS_KEY"), "utf-8")).decode("utf-8")
mlpipeline_minio_secret_key = base64.b64encode(bytes(os.environ.get("MINIO_SECRET_KEY"), "utf-8")).decode("utf-8")
...
class Controller(BaseHTTPRequestHandler):
    def sync(self, parent, children):
        ...
        desired_status = {
            ...
            and len(children["ServiceRole.rbac.istio.io/v1alpha1"]) == 1
            and len(children["ServiceRoleBinding.rbac.istio.io/v1alpha1"]) == 1
            and "True"
            or "False"
        }
        ...
        desired_resources = [
            ...
            # Visualization server related manifests below
            {
                ...
                "spec": {
                    ...
                    "template": {
                        "metadata": {
                            "labels": {"app": "ml-pipeline-visualizationserver"},
                            "annotations": disable_istio_sidecar and {"sidecar.istio.io/inject": "false"} or {},
                        },
                        "spec": {
                            "containers": [
                                {
                                    "image": "gcr.io/ml-pipeline/visualization-server:" + kfp_version,
            ...
            },
            {
                ...
                "spec": {"host": "ml-pipeline-visualizationserver", "trafficPolicy": {"tls": {"mode": "ISTIO_MUTUAL"}}},
            },
            {
                ...
                "spec": {"rules": [{"services": ["ml-pipeline-visualizationserver.*"]}]},
            },
            {
                "apiVersion": "rbac.istio.io/v1alpha1",
                ...
                "spec": {
                    ...
                    "roleRef": {"kind": "ServiceRole", "name": "ml-pipeline-visualizationserver"},
                },
            },
            {
                "apiVersion": "v1",
                    ...
                    "template": {
                        ...
                        "spec": {
                            "containers": [
                                {
                                    "name": "ml-pipeline-ui-artifact",
                                    "image": "gcr.io/ml-pipeline/frontend:" + kfp_version,
                                    "imagePullPolicy": "IfNotPresent",
