...
def generate_test_path(repo_root, kustomize_rpath):
    ...
    test_path = os.path.join(repo_root, "tests", kustomize_rpath, TEST_NAME)
    return test_path
...
def write_go_test(test_path, package_name, package_dir):
    ...
    """
    test_contents = template.render({"package": package_name, "package_dir": package_dir})
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--all", dest="all_tests", action="store_true", help="(Deprecated) this parameter has no effect"
    )
    ...
    loader = jinja2.FileSystemLoader(searchpath=os.path.join(this_dir, "templates"))
