...
def create_subst(name, value, pattern, cwd):
    command = ["kpt", "cfg", "create-subst", ".", name, "--field-value", value, "--pattern", pattern]
    logging.info("Run:\n" + " ".join(command))
    subprocess.check_call(command, cwd=cwd)
...
class KptCreator:
    ...
    def create_subst_asm(path):
        ...
        create_subst(
            "asm-gcp-metadata",
            "project-id|147474701642|name|us-central1-c",
            "${gcloud.core.project}|${gcloud.project.projectNumber}|${name}|${location}",
            path,
        )

        create_subst(
            "asm-cluster-url",
            "https://container.googleapis.com/v1/projects/project-id/locations/us-central1/clusters/name",
            "https://container.googleapis.com/v1/projects/${gcloud.core.project}/locations/${location}/clusters/${name}",
            path,
        )

        create_subst("asm-mesh-id", "project-id_us-central1_name", "${gcloud.core.project}_${location}_${name}", path)

        create_subst("mesh-id", "project-id_us-east1-d_name", "${gcloud.core.project}_${location}_${name}", path)

        create_subst(
            "asm-cluster-name", "project-id/us-central1/name", "${gcloud.core.project}/${location}/${name}", path
        )

    @staticmethod
    def create_subst_private(path):
        registries = ["quay.io/jetstack", "gcr.io/kubeflow-images-public", "metacontroller"]
    ...
    def create_subst_mgmt(path):
        ...
        create_subst(
            "cluster-name", "project-id/mgmt-location/mgmt-name", "${gcloud.core.project}/${location}/${name}", path
        )

        create_subst("pool", "name-pool", "${name}-pool", path)
    ...
    def create_subst(path):
        ...
        create_subst(
            "name-storage-metadata-store", "name-storage-metadata-store", "${name}-storage-metadata-store", path
        )
        create_subst(
            "name-storage-artifact-store", "name-storage-artifact-store", "${name}-storage-artifact-store", path
        )
        create_subst("name-ip", "name-ip", "${name}-ip", path)

        # DNS
        zones = ["gcr", "gcr-cname", "gcr-a", "goog-apis", "goog-cname", "goog-a"]
        for z in zones:
            name = f"name-{z}"
            value = f"name-{z}"
            pattern = f"${{name}}-{z}"
        ...
        create_subst("name-vm-policy-logging", "name-vm-logging", "${name}-vm-logging", path)

        policies = ["monitoring", "meshtelemetry", "cloudtrace", "monitoring-viewer", "storage"]
        ...
        create_subst("cluster-name", "project-id/us-east1-d/name", "${gcloud.core.project}/${location}/${name}", path)

        create_subst("identity-ns", "project-id.svc.id.goog", "${gcloud.core.project}.svc.id.goog", path)

        # Names for service accounts
        create_subst("admin-sa-name", "name-admin", "${name}-admin", path)

        create_subst("user-sa-name", "name-user", "${name}-user", path)

        # Workload identity
        create_subst("name-admin-wi", "name-admin-workload-identity-user", "${name}-admin-workload-identity-user", path)

        create_subst(
            "admin-profiles-sa-wi",
        ...
        create_subst("projects", "projects/project-id", "projects/${gcloud.core.project}", path)
        ...
        create_subst("node-pool-cpu", "name-cpu-pool-v1", "${name}-cpu-pool-v1", path)

        create_subst("name-admin-manages-user", "name-admin-manages-user", "${name}-admin-manages-user", path)
    ...
    def restore():
        subdirs = [
            "gcp/cloud-endpoints",
            "gcp/deployment_manager_configs",
            "gcp/gpu-driver",
            "gcp/iap-ingress",
            "gcp/prometheus",
            "gcp/privateutil",
